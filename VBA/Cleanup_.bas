...
'   Cleanup Auto Toggle
'
Public Function Cleanup_Toggle() As Boolean
Const ThisProc = "Cleanup_Toggle"
Cleanup_Toggle = False
    
    If Msg_Box(Proc:=ThisProc, Buttons:=vbYesNoCancel, Default:=vbDefaultButton3, Icon:=vbQuestion, _
               Text:="glbCleanupAuto is " & UCase(CStr(glbCleanupAuto)) & glbBlankLine & "Change the value to " & UCase(CStr(Not glbCleanupAuto)) & "?") _
        = vbYes Then glbCleanupAuto = Not glbCleanupAuto
    
Cleanup_Toggle = True
End Function

'   Cleanup Manual - Called from QAT or Ribbon
'
...
'   Forward No Cleanup - Called from ProcXeq
'
Public Function Cleanup_ForwardNoCleanup() As Boolean
Const ThisProc = "Cleanup_ForwardNoCleanup"
Cleanup_ForwardNoCleanup = False
    
    '   Get a single Item (Inspector or Explorer)
    '
    Dim oItem As Object
    If Not Misc_GetActiveItem(oItem, ThisProc) Then Exit Function
    
    '   Create the Forward
    '
    Dim oForward As Object
    On Error Resume Next
        Set oForward = oItem.Forward
        Select Case Err.Number
            ...
            Case glbError_PropertyNotFound
                ...
                '   If Object doesn't support Forward - Msg and Bail
                '
                Msg_Box Proc:=ThisProc, Step:="oItem.Forward", _
                Text:="Can not Forward " & _
                      "Item Type '" & Misc_ItemTypeName(oItem) & "'. (Method not supported)."
                Err.Clear
                Exit Function
                
            Case glbError_AppOrObjectDefinedError
            
                '   If oForward got killed by Response Proc - Bail
                '
                If oForward Is Nothing Then
                    Err.Clear
                    Exit Function
                Else
                    Stop: Exit Function
    ...
    '   Set the new Forward Item's InspShadow CleanupResponseBypass = True
    '   Show the new Forward
    '
    Dim InspectorRec As Inspector_InspectorRec
    If Not Inspector_RecFromItem(oForward, InspectorRec) Then Stop: Exit Function
    
    With InspectorRec
    
        .oInspShadow.CleanupResponseBypass = True
        .oInspector.Activate
    
    End With
    
Cleanup_ForwardNoCleanup = True
End Function

'   Cleanup Response - Called from a Response Inspector Activate Event
'
...
Cleanup_Response = False
    ...
    '   If the Global Cleanup Switch is OFF - done
    '
    If Not glbCleanupAuto Then
        Cleanup_Response = True
        Exit Function
    End If
    
    '   If InspShadow.CleanupResponseBypass is TRUE - done
    '
    If Not Inspector_ItemInspectorExist(InspectorItem) Then Stop: Exit Function
    Dim InspectorRec As Inspector_InspectorRec
    If Not Inspector_RecFromItem(InspectorItem, InspectorRec) Then Stop: Exit Function
    
    With InspectorRec
        
        If .oInspShadow.CleanupResponseBypass Then
            Cleanup_Response = True
            Exit Function
        End If
...
'   Cleanup Send - Called from a Send Event
'
'   SPOS - Stupid checks the HTML when the Send event fires and "fixes" it
'   if he doesn't like something in the Response (e.g. No HTML RepSep
'   where he exspects it to be). So running Cleanup manually and running
'   it inside a Send event can give different results.
'
Public Sub Cleanup_Send(ByVal OnSendItem As Object, ByRef Cancel As Boolean)
Const ThisProc = "Cleanup_Send"

    '   If the Global Switch is OFF - done
    '
    If Not glbCleanupAuto Then Exit Sub
    
    '   If a Meeting Response Action (Accept, Decline, Tenative) - done
    '
    If Mail_IsMeetingResponseAction(OnSendItem) Then Exit Sub
    
    '   If CleanupSkipOnSend - done
    '   If CleanupResponseBypass - done
    '
    If Not Inspector_ItemInspectorExist(OnSendItem) Then Exit Sub
    Dim InspectorRec As Inspector_InspectorRec
    If Not Inspector_RecFromItem(OnSendItem, InspectorRec) Then Exit Sub
    
    With InspectorRec
    
        If .oInspShadow.CleanupSkipOnSend Then Exit Sub
        If .oInspShadow.CleanupResponseBypass Then Exit Sub
    ...
    '   2024-09-15 - SPOS - Meeting Accept, Tenative, Decline, Cancel without
    '   any text (Send the responce now) are not shown. So don't even think
    '   about doing Cleanup.
    '
    If Mail_IsMeetingResponse(Item) Then
        If Item.Body = "" Then Exit Sub
    End If
    
    CleanupType = IIf(Mail_IsResponse(Item), CleanupTypes.SendResponse, CleanupTypes.SendNew)
    Dim CleanupMain As Boolean
    CleanupMain = Cleanup_Main()
    
    '   Confirmation or Skip
    '
    Select Case Msg_Box( _
            Buttons:=vbYesNoCancel, Proc:=ThisProc, Step:="After Cleanup Send", _
            Icon:=IIf(CleanupMain, vbQuestion, vbExclamation), _
            Default:=IIf(CleanupMain, vbDefaultButton1, vbDefaultButton2), _
            Text:=CleanupMsg & glbBlankLine & _
                  "Send it?" & Space(glbMsg_MaxWidth - 72) & "(Cancel = No Cleanup next Send)" _
                  )
        Case vbYes
            Cleanup_RepSepRemove
        Case vbNo
            Cancel = True
        Case vbCancel
            Cancel = True
            Cleanup_RepSepRemove
            InspectorRec.oInspShadow.CleanupSkipOnSend = True
            Msg_Box Proc:=ThisProc, Step:="Cancel on Send", Text:="Cleanup will be skipped on the next Send for this Inspector."
    End Select
...
Public Function Cleanup_Main() As Boolean
    ...
    Else
        ...
        Cleanup_Main = True
        InfoText = "SPOS - Meeting Response is RTF that can't be converted to HTML. Can't do Cleanup." & vbNewLine
...
Cleanup_Init = False
    ...
    If Mail_IsRTF(Item) Then
        InfoText = InfoText & "Item is RTF and can't be converted to HTML. Can't do Cleanup." & vbNewLine
        Exit Function
    End If
    ...
    '   Doesn't have at least 3 para
    '
    If wDoc.Content.Paragraphs.count < 3 Then
        InfoText = InfoText & "Paragraph Count < 3. Can't do Cleanup." & vbNewLine
        Exit Function
    End If
...
Cleanup_BQReplace = False
    ...
    Item.HTMLBody = sHTMLBody
        Select Case Err.Number
            Case glbError_None
                ' Continue
            Case glbError_CleanupHTML_Hoarked
                InfoText = InfoText & "Cleanup_BQReplace - Error putting Clean HTML back into the Item (Item.HTMLBody = sHTMLBody)." & vbNewLine
                Exit Function
            Case Else
...
Cleanup_BQRemove = False
    ...
    For Each SearchTag In Array(BQStartTag, BQEndTag)
        ...
        Do
            ...
            If TagStartIx = 0 Then Exit Do  ' Next SearchTag
...
Cleanup_BQRestoreHTML = False
    ...
    For Each SearchChr In Array(BQStartMarkChr, BQEndMarkChr)
        ...
        Do
            ...
            If iX = 0 Then Exit Do  ' Next SearchChr
